...
for action in peek open-browser setup customize install-dependencies doctor clear-cache; do
  grep -Fqx "id = \"$action\"" "$MANIFEST" \
    || fail "manifest action $action"
done
for script in peek.sh open-browser.sh setup.sh setup-action.sh setup-wizard.sh customize-action.sh customize.sh install-dependencies-action.sh install-dependencies.sh doctor.sh clear-cache.sh viewer.sh; do
